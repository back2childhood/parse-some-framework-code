/* ************************************************************************
> File Name:     TimeWheel.c
> Author:        Qian JiLi
> mail:          193937157@qq.com
> Created Time:  Wed May  3 22:55:19 2023
> Description:
 ************************************************************************/
/*
...
static void internal_add_timer(struct tvec_base *base,
  ...
  {
    vec = base->tv1.vec +
          (base->current_index & TVR_MASK); /*放到第一个轮的当前槽*/
...
void *ti_add_timer(void *ptimewheel, unsigned long expires,
                   timeouthandle phandle, unsigned long arg) {
...
static int cascade(struct tvec_base *base, struct tvec *tv, int index) {
  ...
  list_for_each_safe(pos, tmp,
                     &tv_list) /*遍历tv_list双向链表，将任务重新添加到时间轮*/
  {
    timer = list_entry(pos, struct timer_list,
                       entry); /*struct timer_list中成员entry的地址是pos,
                                  获取struct timer_list的首地址*/
...
static void *deal_function_timeout(void *base) {
  ...
  for (;;) {
    ...
    while (ba->current_index <=
           (tv.tv_sec * 1000 + tv.tv_usec / 1000)) /*单位：ms*/
    {
      struct list_head work_list;
      int index = ba->current_index & TVR_MASK; /*获取第一个轮上的指针位置*/
      struct list_head *head = &work_list;
      /*指针指向0槽时，级联轮需要更新任务列表*/
      if (!index && (!cascade(ba, &ba->tv2, INDEX(0))) &&
